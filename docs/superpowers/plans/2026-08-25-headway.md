...
test("readVarint survives values above 32 bits", () => {
  // 1787682449 (a real feed timestamp) must not be truncated by bitwise ops
  const bytes = u8(0xd1, 0xe4, 0xb7, 0xd4, 0x06)
...
test("decodeTripUpdates surfaces express route ids from the numbered feed", () => {
  const feed = Gtfs.decodeTripUpdates(fixture("gtfs.pb"))
  const ids = new Set(feed.trips.map((t) => t.routeId))
  // The numbered feed carries 1-7 plus GS; 6X/7X appear when expresses run.
  assert.ok(ids.has("6") || ids.has("6X"), "the 6 is present in some form")
...
test("decodeAlerts reads every alert with text and routes", () => {
  ...
  assert.ok(feed.alerts.length > 50, "the alerts feed is well populated")
...
test("search groups stations sharing a complex adjacently", () => {
  const results = Stations.search("Times Sq", null, 20)
  const ids = results.map((s) => s.complexId)
  const first = ids.indexOf(ids[0])
  const last = ids.lastIndexOf(ids[0])
  assert.equal(last - first + 1, ids.filter((c) => c === ids[0]).length,
    "same-complex rows are contiguous")
})
  ...
  function refresh() {
    ...
    for (var i = 0; i < feeds.length; i++) {
      fetchBytes(Gtfs.feedUrl(feeds[i]), function (bytes) {
        var decoded = Gtfs.decodeTripUpdates(bytes)
