...
In QML this file is loaded as `import "StationData.js" as StationData` from within `Stations.js` — which QML does **not** support. So `Stations.js` reads a module-level `STATIONS` that the QML side supplies via `Stations.load(StationData.STATIONS)`, and node supplies via `require`. This one seam is why `Stations.js` has an initializer where the other pure modules do not.
...
Item {
  ...
  Component.onCompleted: {
    Stations.load(StationData.STATIONS)
    root.nowSec = Math.floor(Date.now() / 1000)
  }
  ...
  readonly property var station: root.saved ? Stations.byId(root.saved.stopId) : null
  ...
  function checkNewAlerts() {
    ...
    for (var i = 0; i < live.length; i++) {
      ...
      if (root.seenAlertIds[a.id]) continue
  ...
  // A QUEUE, not a single pending slot. Colophon uses one slot and says why
  // that is enough there: "there is only one notification type, so a burst is
  // not reachable... add galley's full queue if you add a second type."
  // Headway has two — a new route alert and a stale feed — and both can fire
  // in the same poll, so one would be silently dropped by a single slot.
    ...
    ColumnLayout {
        ...
        delegate: RowLayout {
          ...
          Text {
            ...
            text: {
              ...
              var dest = Stations.byId(terminal)
      ...
      Repeater {
        ...
        delegate: RowLayout {
          ...
          Button {
            text: (Stations.byId(savedRow.modelData.stopId)
      ...
      Repeater {
        model: Stations.search(root.query, service.origin, 6)
        delegate: RowLayout {
          id: hit
          required property var modelData
          Layout.fillWidth: true
