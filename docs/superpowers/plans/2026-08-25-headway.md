...
function classifyAlert(alertType) {
  ...
  if (ALERT_RED[alertType]) return "red"
  if (ALERT_AMBER[alertType]) return "amber"
...
Item {
  ...
  // every request carries an explicit abort timer. This path is also what the
  // unreachable-feed state depends on, so it is not merely defensive.
  function fetchBytes(url, onOk, onFail) {
    var xhr = new XMLHttpRequest()
    var settled = false
    var timer = Qt.createQmlObject(
      'import QtQuick; Timer { interval: 15000; running: true }', root)
  ...
  // A single pending slot, not a queue: assigning Process.command while it is
  // still running is a silent no-op, so a burst would otherwise vanish.
  ...
  Process {
    ...
    onRunningChanged: {
      ...
      if (running) return
      if (root.pendingNotification === null) return
  ...
  function notify(summary, body) {
    var cmd = ["notify-send", "-a", "Headway", summary, body]
    if (notifyProc.running) { root.pendingNotification = cmd; return }
...
Expected: PASS, 3 tests. This is where Task 10's deferred test finally goes green.
