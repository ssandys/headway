...
- **Export shim, at the tail of every pure module, verbatim:**
  ...
  ```
- **`BAR_GLYPH` is `String.fromCodePoint(0xF1308)`.** Never paste the literal character — spec finding 8 proves it does not survive the editing path.
...
```js
// md-account_tie_voice. Built with fromCodePoint, NEVER pasted as a literal: a
...
Item {
  ...
  FileView {
    ...
    // A self-write fires this watcher and FileView.text() is still the OLD
    // content at that moment, so an unguarded reload undoes the save. The flag
    // is consumed, not latched, so external edits still work.
    onFileChanged: {
...
Panel {
  ...
  KeyboardPanel {
    ...
    ColumnLayout {
      id: contentColumn
      // REQUIRED: without these the column takes its own implicitWidth (the
      // widest child = an unwrapped alert headline) while the background stays
      // at contentWidth, so content paints outside the panel. Left/right/top,
      // never anchors.fill -- pinning the bottom loops against contentHeight,
      // which is bound back to contentColumn.implicitHeight.
...
Then: Install (`omarchy plugin add https://github.com/ssandys/headway.git --enable`); Reading the bar (the severity table from the spec); Using the panel (keys and interactions); Configuration (the settings table from the spec); Troubleshooting (`omarchy restart shell` after a structure change; `node scripts/collect.mjs` for the raw snapshot; what a stale glyph means); Known limitations (from the spec's "Out of scope", **plus the one below**); Uninstall.

**Add this limitation, which is not in the spec's out-of-scope list because it was found late:**

> **One poll per monitor.** The Omarchy bar instantiates a widget once per
> bar surface, and a bar surface exists per monitor — so on a two-monitor
> setup Headway fetches the feeds twice per interval, on three monitors three
> times. Neither galley nor colophon coordinates across instances either
> (verified: their poll timers run unconditionally per instance), so this is
> the house behaviour rather than a Headway bug. It matters slightly more
> here because those two poll local services while this one polls the MTA
> over the network. The volume is small — riding only the L is 23 KB per
> fetch — but it is real, and worth knowing before running Headway on a wall
> of monitors.

State it as a limitation rather than a defect, and do not invent a
coordination scheme to avoid it — deciding which instance owns the poll, and
what happens when that instance's monitor is unplugged, is materially more
complexity than the duplicate fetch costs.
