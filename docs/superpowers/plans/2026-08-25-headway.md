...
function savedStations() {
  ...
  if (stopId) return { activeStationId: stopId,
...
Item {
  ...
  readonly property var bar: Model.barState(root.snapshot, root.nowSec)
...
**Every name below was verified against the live shell**, because `qmllint` cannot resolve `qs.Ui` or `qs.Commons` and would pass a typo silently. In particular: `BarWidget` has **no** `barForeground` property (read `bar.foreground` with a `Color.foreground` fallback, as colophon does); spacing is the **function** `Style.space(px)`; fonts are `Style.font.caption` / `.body` / `.title`; and there is no muted-colour token at all — secondary text is the foreground at reduced `opacity`.
...
BarWidget {
  ...
  // BarWidget exposes `bar`, not `barForeground`. Colophon reads it exactly
  // this way, with a fallback for when the widget is not yet parented.
  readonly property color fg: barRoot.bar ? barRoot.bar.foreground : Color.foreground

  // Declared on the ROOT, because the search field and the results Repeater
  // both address it as `barRoot.query`. QML resolves an unqualified name
  // against the object, the component root and declared ids -- never an
  // arbitrary enclosing item -- so declaring this on the inner ColumnLayout
  // would fail at runtime as a ReferenceError that qmllint cannot see.
  property string query: ""

  Service {
    id: service
    settings: barRoot.settings
    panelOpen: panel.visible
  }

  WidgetButton {
    id: button
    text: Model.BAR_GLYPH + (service.bar.badge ? "  " + service.bar.badge : "")
    ...
    color: service.bar.severity === "error" ? Model.COLOR_ERROR
         : service.bar.severity === "warn" ? Model.COLOR_WARN
         : barRoot.fg
    onPressed: function (buttonCode) {
      if (buttonCode === Qt.MiddleButton) { service.refresh(); return }
      panel.toggle()
    }
  }
  ...
  KeyboardPanel {
    ...
    ColumnLayout {
      ...
      TextField {
        ...
        onTextEdited: barRoot.query = searchField.text
      ...
      Repeater {
        model: Stations.search(barRoot.query, service.origin, 6)
        delegate: RowLayout {
          id: hit
          required property var modelData
          Layout.fillWidth: true
