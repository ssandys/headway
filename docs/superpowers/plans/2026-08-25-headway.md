...
- Produces: `byId(id)`, `platformId(id, dir)`, `parentOf(platformId)`, `haversineKm(lat1, lon1, lat2, lon2)`, `search(query, origin, limit)`, `directionsFor(station)`, `boroughName(code)`
