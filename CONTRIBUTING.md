...
| **`active_period` filtering is mandatory** | 151 of 195 alerts are `Planned - ` engineering work, much of it for weekends days away. Unfiltered, the glyph pins amber forever. At the fixture's own header timestamp (1787689797) the 195 decode to **9 active**, of which 1 is amber and none red. |
...
| **`FileView` cannot be made write-only, and cannot bound a read** | MEASURED: it attempts a read whenever `path` is assigned, and NEITHER `preload: false` NOR `blockAllReads: true` prevents it -- a probe logged `Read of ... failed: File does not exist` under both. Its whole API is `blockWrites`, `atomicWrites`, `watchChanges`, `adapter`, `path`, `text`, `data`, `preload`, `loaded`, `blockLoading`, `blockAllReads`, `printErrors` -- no size cap, no stat, no symlink control -- and Quickshell exports no filesystem primitive either. Given the sibling property is `blockLoading`, `blockAllReads` most likely means "make reads BLOCKING", which is worse in a shared shell, not better. This is why the state file is read and written through `Process` instead. |
| **`StdioCollector` needs `waitForEnd: true`** | Without it `onStreamFinished` never delivers a complete payload, and the handler simply never fires -- silently. Use bare `text`, not `this.text`. Both first-party users set it: `Commons/Style.qml:446`, `plugins/panels/disk-speedtest/Panel.qml:105`. |
| **Two `Component.onCompleted` handlers on one component** | QML rejects the duplicate and the component fails to instantiate -- with nothing in the journal. Fold new startup work into the existing handler. |
